...
Windows 11 での導入は、このリポジトリでは確認していません。Android 実機ではポインターとキーの押下・解放を確認しています。iOSシミュレータではタップ、ドラッグ、ホームへ移ったあとの復帰と、そのあとの新しいドラッグ、日本語表示を確認しています。iOSの複数指と、接触中のキャンセルは未確認です。

実機への書き出しは、メニューの `GameCanvas/アプリをビルドする` と、[講義テキスト](https://github.com/sfc-sdp/SDP-Textbook) を見てください。

## 配布

配布 ZIP はコミット済みのファイルだけから作ります。

```bash
python3 scripts/package-project.py
```

未コミットの変更や未追跡ファイルがあると拒否します。`design/` や `scripts/rendering/` が Git で追跡されているときも拒否します。出力は `Build/Distribution/GameCanvas-<版>-<短縮リビジョン>.zip` と、同じ名前の `.sha256`、ZIP 内の `release-info.json` です。Unity は 6000.6.2f1、パッケージ版は 8.0.0-pre.1 です。
