Shader "Custom/GameCanvas/DrawImage" {
	...
	SubShader {
		Pass {
			...
			{
				half2 pm = i.uv * _MainTex_TexelSize.zw;              // 処理するピクセル座標
				half2 pi = mul(_Matrix, float4(pm, 0, 1)) + _Clip.xy; // 対応する画像データのピクセル座標
				half2 si = _ImageTex_TexelSize.zw - _Clip.zw;         // 右下の画像ピクセル座標
