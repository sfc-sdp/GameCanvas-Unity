...
public static class Program
{
    public static int Main(string[] args)
    {
        ...
        // Only this dedicated generated directory is cleaned; retired API pages must not survive.
        foreach (var old in Directory.EnumerateFiles(output, $"{namespaceRoot}.*.md")) File.Delete(old);
        var manifest = merged.OrderBy(t => t.FullName).Select(t => new
        {
            name = t.FullName, kind = t.Kind.ToString(), declaration = t.DeclarationSyntax,
            members = t.Members.OrderBy(m => m.Id).Select(m => new { id = m.Id, name = m.Name, signature = m.Signature })
        });
        File.WriteAllText(Path.Combine(output, "api.json"), System.Text.Json.JsonSerializer.Serialize(manifest,
