...
原点は左上です。右が X の正方向、下が Y の正方向です。単位はキャンバスの画素です。描画と座標の回転の引数名は `rotation` です。基準点を中心に時計回りで、単位は度です。カメラから取る角度の出力も `rotation` です。`gc.Sin` や `gc.Cos` の `degree` は角度そのものを指すので、名前はそのままです。
